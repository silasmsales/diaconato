import{$n as yI,B as LD,C as FI,E as Gs,Et as Ug,Fn as sI,Gn as w_,L as Kf,M as J,O as H,Ot as V,Q as Of,St as Tg,U as MD,Y as OC,Zt as bg,at as Pf,en as bt$1,er as y_,ft as S,h as Dm,ht as Sl,jt as Vh,kn as po,kt as VD,l as Bg,lt as RD,n as $g,nr as zD,o as AI,q as Nl,qt as _g,rr as zf,rt as PI,sn as fD,tt as PC,ut as Rg,wt as UI,x as FD,xt as Tb}from"./chunk-DcId_B8o.js";import{a as tn,n as K,r as pa,t as m}from"./main-W2GZ7EP4.js";import{t as m$1}from"./chunk-fRxZRpXP.js";import{t as w}from"./chunk-Diy2ERPT.js";import{_ as ln,d as Vt$1,f as Xe$1,n as Gn,r as Hn,s as Pn,t as Bn}from"./chunk-BcNSYFu0.js";import{t as f}from"./chunk-DzMHVgfb.js";import{i as p,n as h,r as l}from"./chunk-DIO8w6AN.js";import{r as t,t as b}from"./chunk-DYullDaB.js";import{t as f$1}from"./chunk-yZA8dFnA.js";import{t as q}from"./chunk-CHOhVwdM.js";import{t as o}from"./chunk-uz4g6ehC.js";var se=class a{getDiaSemanaFromData(t){if(!t)return``;let e=t.split(`-`);return o[new Date(Number(e[0]),Number(e[1])-1,Number(e[2])).getDay()+1]||``}formatDateBR(t){if(!t)return``;let e=t.split(`-`);return e.length===3?`${e[2]}/${e[1]}/${e[0]}`:t}agruparEventosPorSemana(t){if(!t||t.length===0)return[];let e=new Map;for(let r of t){if(!r.data)continue;let d=r.data.split(`-`),b=new Date(Number(d[0]),Number(d[1])-1,Number(d[2])),f=b.getDay(),h=new Date(b);h.setDate(b.getDate()-f);let E=new Date(h);E.setDate(h.getDate()+6);let m=h.toISOString().split(`T`)[0],S=`${String(h.getDate()).padStart(2,`0`)}/${String(h.getMonth()+1).padStart(2,`0`)}`,p=`${String(E.getDate()).padStart(2,`0`)}/${String(E.getMonth()+1).padStart(2,`0`)}`;e.has(m)||e.set(m,{startFmt:S,endFmt:p,startKey:m,eventos:[]}),e.get(m).eventos.push(r)}return Array.from(e.values()).sort((r,d)=>r.startKey.localeCompare(d.startKey)).map((r,d)=>({titulo:`SEMANA ${d+1} (${r.startFmt} a ${r.endFmt})`,eventos:r.eventos.sort((b,f)=>{let h=(b.data||``).localeCompare(f.data||``);return h!==0?h:(b.turno||0)-(f.turno||0)})}))}openPrintWindow(t,e){let n=window.open(``,`_blank`,`width=1000,height=800`);if(!n){alert(`Por favor, permita pop-ups para gerar o PDF da escala.`);return}`${window.location.origin}`;let d=`
      <!DOCTYPE html>
      <html lang="pt-BR">
      <head>
        <meta charset="UTF-8">
    ...
    `;this.openPrintWindow(`ADTAG_Escala_${r.replace(`/`,`_`)}_Por_Cultos`,E)}gerarPdfPorObreiros(t,e,n,r){let d=l(t),b=Number(t.id_mes),f=`${window.location.origin}/logo_adtag.jpg`,h=(n||[]).filter(p=>Number(p.id_mes)===b),E=new Map;(r||[]).filter(p=>p.ativo).forEach(p=>{E.set(Number(p.id_obreiro),{obreiro:p,escalas:[]})});for(let p of h){let F=Number(p.id_obreiro),I=e.find(O=>Number(O.id_evento)===Number(p.id_evento));I&&(E.has(F)||p.obreiros&&E.set(F,{obreiro:p.obreiros,escalas:[]}),E.has(F)&&E.get(F).escalas.push({data:I.data,turno:I.turno,evento:I.descricao||`Culto`}))}let S=`
      <div class="header-container">
